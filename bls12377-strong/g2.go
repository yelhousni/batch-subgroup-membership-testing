...
func (p *G2Jac) mulBySeed(q *G2Jac) *G2Jac {
	// mulBySeed computation is derived from the addition chain:
	//
	//	_10    = 2*1
	//	_100   = 2*_10
	//	_101   = 1 + _100
	...
	// Step 22: t3 = q^0q40b00
	for s := 0; s < 8; s++ {
		t3.DoubleAssign()
	}

	// Step 23: t2 = q^0q40b0b
	t2.AddAssign(t3)

	// Step 30: t2 = q^0q2058580
	for s := 0; s < 7; s++ {
		t2.DoubleAssign()
	}

	// Step 31: t2 = q^0q205858d
	t2.AddAssign(t0)

	// Step 37: t2 = q^0q81616340
	for s := 0; s < 6; s++ {
		t2.DoubleAssign()
	}

	// Step 38: t1 = q^0q81616347
	t1.AddAssign(t2)

	// Step 45: t1 = q^0q40b0b1a380
	for s := 0; s < 7; s++ {
		t1.DoubleAssign()
	}

	// Step 46: t0 = q^0q40b0b1a38d
	t0.AddAssign(t1)

	// Step 47: t0 = q^0q816163471a
	t0.DoubleAssign()

	// Step 48: &result = q^0q816163471f
	result.AddAssign(t0)

	// Step 72: &result = q^0q816163471f000000
	for s := 0; s < 24; s++ {
		result.DoubleAssign()
	}

	// Step 73: &result = q^0q816163471f000001
	result.AddAssign(q)

	return p.Neg(&result) // negative seed
}
