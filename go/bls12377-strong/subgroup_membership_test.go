...
func TestIsInSubGroupBatch(t *testing.T) {
	...
	properties.Property("[BLS12-377-STRONG] IsInSubGroupBatch test should pass with high probability", prop.ForAll(
		func(mixer fr.Element) bool {
			// mixer ensures that all the words of a frElement are set
			var sampleScalars [nbSamples]fr.Element
	...
	properties.Property("[BLS12-377-STRONG] IsInSubGroupBatch test should not pass with high probability", prop.ForAll(
		func(mixer fr.Element, a fp.Element) bool {
			// mixer ensures that all the words of a frElement are set
			var sampleScalars [nbSamples]fr.Element
...
func TestTatePairings(t *testing.T) {
	...
	properties.Property("[BLS12-377-STRONG] Tate(P2,Q) and Tate(P'2,Q) should both be 1", prop.ForAll(
		func(a fr.Element) bool {
			var s big.Int
			a.BigInt(&s)
			_, _, g, _ := Generators()
